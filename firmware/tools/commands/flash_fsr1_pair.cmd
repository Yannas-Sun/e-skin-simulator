...
echo [STM32] Building and flashing the complete FSR1 16x16 firmware...
call "%~dp0original\flash_stm32_project.cmd" fsr1
...
echo [GUI] Opening the complete FSR1 16x16 heatmap on %PORT%...
python -u "%ROOT%\teensy\applications\ESKIN_SPI_USB_BRIDGE\live_fsr_heatmap.py" ^
  --port "%PORT%" --region fsr1
exit /b %errorlevel%
