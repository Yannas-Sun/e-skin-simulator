...
/* FSR2 frame: MUX2 selections x ADC2 channels, 12-bit samples in 16 bits. */
#define FSR_ROWS             16U
...
static void MUX2_SelectRow(uint8_t row);
static HAL_StatusTypeDef MAX11633_ADC2_Init(void);
static HAL_StatusTypeDef MAX11633_ADC2_Read16(uint16_t *samples);
...
int main(void)
{
  ...
  /* Isolate FSR1 and leave FSR2 disabled until each settled acquisition. */
  HAL_GPIO_WritePin(GPIOA, MUX_EN1_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(GPIOB, ADC_CS1_Pin|MUX_EN2_Pin|ADC_CS2_Pin, GPIO_PIN_SET);
  fsr_scan_status = MAX11633_ADC2_Init();
  ...
  while (1)
  {
    ...
    else
    {
      ...
      ++fsr_error_count;
      HAL_GPIO_WritePin(GPIOA, MUX_EN1_Pin, GPIO_PIN_SET);
      HAL_GPIO_WritePin(GPIOB, MUX_EN2_Pin|ADC_CS1_Pin|ADC_CS2_Pin,
                        GPIO_PIN_SET);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, ADC_CS1_Pin|ADC_CS2_Pin|MUX_EN2_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(GPIOB, HOST_IRQ_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : ADC_CS1_Pin ADC_CS2_Pin MUX_EN2_Pin HOST_IRQ_Pin */
  GPIO_InitStruct.Pin = ADC_CS1_Pin|ADC_CS2_Pin|MUX_EN2_Pin|HOST_IRQ_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  /*Configure GPIO pins : ADC_EOC1_Pin ADC_EOC2_Pin HOST_SYNC_Pin */
  GPIO_InitStruct.Pin = ADC_EOC1_Pin|ADC_EOC2_Pin|HOST_SYNC_Pin;
...
static HAL_StatusTypeDef MAX11633_ADC2_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, ADC_CS2_Pin, GPIO_PIN_RESET);
  status = HAL_SPI_Transmit(&hspi1, (uint8_t *)&reset_command, 1U,
                            MAX11633_EOC_TIMEOUT);
  HAL_GPIO_WritePin(GPIOB, ADC_CS2_Pin, GPIO_PIN_SET);
  if (status != HAL_OK)
  {
    return status;
  }
...
static HAL_StatusTypeDef MAX11633_ADC2_Read16(uint16_t *samples)
{
  ...
  start = HAL_GetTick();
  while (HAL_GPIO_ReadPin(GPIOB, ADC_EOC2_Pin) == GPIO_PIN_SET)
  {
...
{
  HAL_StatusTypeDef status;
  uint16_t adc2_samples[FSR_COLUMNS];

  for (uint8_t mux_index = 0U; mux_index < FSR_ROWS; ++mux_index)
  {
    const uint8_t mux2_channel = (uint8_t)(FSR_ROWS - 1U - mux_index);

    fsr_active_row = mux_index;
    /*
     * FSR2 is physically reversed relative to FSR1. Select MUX2 in reverse
     * order so the transmitted frame retains the established logical R/C map.
     */
    MUX2_SelectRow(mux2_channel);
    ...
    status = MAX11633_ADC2_Read16(adc2_samples);
    ...
    /*
     * FSR2 exchanges the physical row/column axes. ADC2 channel order is kept
     * direct on frame axis 0 to correct the physical left/right direction;
     * MUX2 remains reversed on frame axis 1. Coordinates and GUI geometry are
     * unchanged: frame[physical column][physical row].
     */
    for (uint8_t adc_index = 0U; adc_index < FSR_COLUMNS; ++adc_index)
    {
      frame[adc_index][mux_index] = adc2_samples[adc_index];
    }
  }
  return HAL_OK;
}
