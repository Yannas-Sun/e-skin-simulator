...
#define ACC_SCK_HALF_PERIOD_MS  1000U
...
static void ACC_SCKSlowInit(void);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    /* One second at each rail makes the shared SCK path meter-visible. */
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(ACC_ENABLE_GPIO_Port, ACC_ENABLE_Pin, GPIO_PIN_SET);
    HAL_Delay(ACC_SCK_HALF_PERIOD_MS);
...
static void ACC_SCKSlowInit(void)
{
  ...
  /* Remove SPI2 alternate functions so SCK can be toggled as a slow GPIO. */
  (void)HAL_SPI_DeInit(&hspi2);

  /* Keep MOSI low; all nCS outputs stay inactive throughout this test. */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);
  ...
  /* Disable the decoder: every sensor CS remains high while SCK changes. */
  ACC_DeselectAll();
}
