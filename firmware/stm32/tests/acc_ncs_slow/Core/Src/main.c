...
#define ACC_NCS_HALF_PERIOD_MS  1000U
...
#error "ACC_SELECTED must be 1..9 for the slow nCS test"
#endif
...
static void ACC_NCSSlowInit(void);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    /* Keep SCK low and alternate only the selected decoder nCS output. */
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);
    ACC_DeselectAll();
    HAL_Delay(ACC_NCS_HALF_PERIOD_MS);
...
static void ACC_NCSSlowInit(void)
{
  ...
  /* Keep both SCK and MOSI low; only nCS is allowed to change. */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);
