...
/* Fixed-address isolation test: physical ACC1 is decoder address 0 and
 * physical ACC5 is decoder address 4. Each output frame contains exactly
 * 100 WHO_AM_I transactions for ACC1 followed by 100 for ACC5. */
#define ACC_TEST_A1_INDEX       0U
#define ACC_TEST_A5_INDEX       4U
#define ACC_TEST_READS          100U
#define ACC_TEST_A1_OFFSET      0U
#define ACC_TEST_A5_OFFSET      100U
#define ACC_TEST_SUMMARY_OFFSET 200U
#define ACC_TEST_SPI_ERROR_BASE 0x0100U
#define ACC_TEST_MAGIC          0x0A15U
...
int main(void)
{
  ...
  /* Do not configure or scan the other seven devices in this isolation test. */
...
static HAL_StatusTypeDef ACC_InitAll(void)
{
  ...
  ACC_DeselectAll();
  HAL_Delay(20U);
  return (acc_ready_count == ACC_COUNT) ? HAL_OK : HAL_ERROR;
}
...
static HAL_StatusTypeDef ACC_RetryFailed(void)
{
  ...
  ACC_DeselectAll();
  return (ready_count == ACC_COUNT) ? HAL_OK : HAL_ERROR;
}

static HAL_StatusTypeDef ACC_ScanFrame(
    uint16_t frame[FSR_ROWS][FSR_COLUMNS])
{
  uint16_t *flat = &frame[0][0];
  uint16_t a1_pass = 0U;
  uint16_t a1_zero = 0U;
  uint16_t a1_ff = 0U;
  uint16_t a1_other = 0U;
  uint16_t a1_spi_error = 0U;
  uint16_t a5_pass = 0U;
  uint16_t a5_zero = 0U;
  uint16_t a5_ff = 0U;
  uint16_t a5_other = 0U;
  uint16_t a5_spi_error = 0U;

  memset(frame, 0, sizeof(uint16_t) * FSR_ROWS * FSR_COLUMNS);

  /* Hold the decoder address at A1 and perform 100 independent 16-clock
   * WHO_AM_I transactions. nCS is released between transactions. */
  for (uint16_t sample = 0U; sample < ACC_TEST_READS; ++sample)
  {
    uint8_t identity = 0U;
    const HAL_StatusTypeDef status =
        ACC_ReadRegister(ACC_TEST_A1_INDEX, ACC_WHO_AM_I_REG, &identity);
    if (status != HAL_OK)
    {
      flat[ACC_TEST_A1_OFFSET + sample] =
          (uint16_t)(ACC_TEST_SPI_ERROR_BASE | ((uint16_t)status & 0x000FU));
      ++a1_spi_error;
    }
    else
    {
      flat[ACC_TEST_A1_OFFSET + sample] = identity;
      if (identity == ACC_WHO_AM_I_EXPECTED) ++a1_pass;
      else if (identity == 0x00U) ++a1_zero;
      else if (identity == 0xFFU) ++a1_ff;
      else ++a1_other;
    }
  }
  ...
  /* Repeat the same test at decoder address A5. No other ACC address is used. */
  for (uint16_t sample = 0U; sample < ACC_TEST_READS; ++sample)
  {
    uint8_t identity = 0U;
    const HAL_StatusTypeDef status =
        ACC_ReadRegister(ACC_TEST_A5_INDEX, ACC_WHO_AM_I_REG, &identity);
    if (status != HAL_OK)
    {
      flat[ACC_TEST_A5_OFFSET + sample] =
          (uint16_t)(ACC_TEST_SPI_ERROR_BASE | ((uint16_t)status & 0x000FU));
      ++a5_spi_error;
    }
    else
    {
      flat[ACC_TEST_A5_OFFSET + sample] = identity;
      if (identity == ACC_WHO_AM_I_EXPECTED) ++a5_pass;
      else if (identity == 0x00U) ++a5_zero;
      else if (identity == 0xFFU) ++a5_ff;
      else ++a5_other;
    }
  }

  ACC_DeselectAll();
  flat[ACC_TEST_SUMMARY_OFFSET + 0U] = ACC_TEST_MAGIC;
  flat[ACC_TEST_SUMMARY_OFFSET + 1U] = a1_pass;
  flat[ACC_TEST_SUMMARY_OFFSET + 2U] = a1_zero;
  flat[ACC_TEST_SUMMARY_OFFSET + 3U] = a1_ff;
  flat[ACC_TEST_SUMMARY_OFFSET + 4U] = a1_other;
  flat[ACC_TEST_SUMMARY_OFFSET + 5U] = a1_spi_error;
  flat[ACC_TEST_SUMMARY_OFFSET + 6U] = a5_pass;
  flat[ACC_TEST_SUMMARY_OFFSET + 7U] = a5_zero;
  flat[ACC_TEST_SUMMARY_OFFSET + 8U] = a5_ff;
  flat[ACC_TEST_SUMMARY_OFFSET + 9U] = a5_other;
  flat[ACC_TEST_SUMMARY_OFFSET + 10U] = a5_spi_error;
  flat[ACC_TEST_SUMMARY_OFFSET + 11U] = acc_idle_miso[ACC_TEST_A1_INDEX];
  flat[ACC_TEST_SUMMARY_OFFSET + 12U] = acc_idle_miso[ACC_TEST_A5_INDEX];
  flat[ACC_TEST_SUMMARY_OFFSET + 13U] = acc_command_rx[ACC_TEST_A1_INDEX];
  flat[ACC_TEST_SUMMARY_OFFSET + 14U] = acc_command_rx[ACC_TEST_A5_INDEX];
  flat[ACC_TEST_SUMMARY_OFFSET + 15U] =
      (uint16_t)((acc_spi_error[ACC_TEST_A1_INDEX] |
                  acc_spi_error[ACC_TEST_A5_INDEX]) & 0x0FFFU);
  return HAL_OK;
}
