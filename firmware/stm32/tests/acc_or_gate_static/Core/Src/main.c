...
/*
 * Build-time ACC selection supplied by CMake:
 *   0    scan all nine installed devices (normal array test)
 *   1..9 initialise, retry, and read only that physical ACC
 */
#ifndef ACC_SELECTED
#define ACC_SELECTED 1
#endif
#if (ACC_SELECTED < 1) || (ACC_SELECTED > ACC_COUNT)
#error "ACC_SELECTED must be 1..9 for the OR-gate static test"
#endif
#define ACC_TARGET_COUNT 1U
#ifndef OR_GATE_MOSI_LEVEL
#define OR_GATE_MOSI_LEVEL 1
#endif
#if (OR_GATE_MOSI_LEVEL != 0) && (OR_GATE_MOSI_LEVEL != 1)
#error "OR_GATE_MOSI_LEVEL must be 0 or 1"
#endif
...
static void ACC_ORGateStaticInit(void);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    /* Maintain DC levels for multimeter probing; no SPI clock is generated. */
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);
#if OR_GATE_MOSI_LEVEL == 1
...
static void ACC_ORGateStaticInit(void)
{
  ...
  /* Remove SPI2 alternate functions so SCK and MOSI become static GPIOs. */
  (void)HAL_SPI_DeInit(&hspi2);

  /* SCK stays low, so holding nCS low cannot create an SPI transaction. */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);
#if OR_GATE_MOSI_LEVEL == 1
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_15, GPIO_PIN_SET);
#else
  ...
  /* Address ACC1..ACC9 as Y0..Y8, then keep the selected nCS low forever. */
  ACC_Select((uint8_t)(ACC_SELECTED - 1U));
}
