...
#define ACC_MOSI_HALF_PERIOD_MS  1000U
 ...
 */
#ifndef ACC_SELECTED
#define ACC_SELECTED 3
#endif
#if (ACC_SELECTED < 1) || (ACC_SELECTED > ACC_COUNT)
#error "ACC_SELECTED must be 1..9 for the slow MOSI test"
#endif
...
static void ACC_MOSISlowInit(void);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    /* Keep SCK and the selected nCS low; alternate only shared MOSI. */
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_15, GPIO_PIN_RESET);
    HAL_Delay(ACC_MOSI_HALF_PERIOD_MS);
...
static void ACC_MOSISlowInit(void)
{
  ...
  /* Remove SPI2 alternate functions: this test must generate no SPI clocks. */
  (void)HAL_SPI_DeInit(&hspi2);

  /* Start with SCK and MOSI low; only MOSI changes after selection. */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_15, GPIO_PIN_RESET);
  ...
  /* Start with every sensor deselected. */
  ACC_DeselectAll();
}
