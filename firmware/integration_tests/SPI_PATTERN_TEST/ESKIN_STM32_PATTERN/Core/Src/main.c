...
/* Basic blocking scan implementation; DMA can be added after bring-up. */
...
static uint8_t host_rx_frame[HOST_FRAME_BYTES];
static uint16_t host_sequence;
static volatile HAL_StatusTypeDef fsr_scan_status = HAL_OK;

/* Isolated STM32-to-Teensy SPI3 link test. */
#define SPI_PATTERN_BYTE       0x55U
#define SPI_PATTERN_TIMEOUT_MS 1000U

static uint8_t spi_pattern_tx = SPI_PATTERN_BYTE;
static uint8_t spi_pattern_rx;
static volatile HAL_StatusTypeDef spi_pattern_status = HAL_OK;
...
int main(void)
{
  ...
  /* Complete SPI pattern test: IRQ announces that SPI3 is ready. */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    spi_pattern_tx = SPI_PATTERN_BYTE;
    spi_pattern_rx = 0U;

    /*
     * The STM32 is the SPI3 slave. HOST_IRQ goes high only after the HAL
     * transfer is about to wait for the Teensy master to assert NSS and
     * generate eight clocks.
     */
    HAL_GPIO_WritePin(GPIOB, HOST_IRQ_Pin, GPIO_PIN_SET);
    spi_pattern_status = HAL_SPI_TransmitReceive(
        &hspi3,
        &spi_pattern_tx,
        &spi_pattern_rx,
        1U,
        SPI_PATTERN_TIMEOUT_MS);
    HAL_GPIO_WritePin(GPIOB, HOST_IRQ_Pin, GPIO_PIN_RESET);

    if (spi_pattern_status != HAL_OK)
    {
      (void)HAL_SPI_Abort(&hspi3);
    }

    /* Provide a visible low interval on HOST_IRQ between transactions. */
    HAL_Delay(10U);
  }
  /* USER CODE END 3 */
}
...
static HAL_StatusTypeDef MAX11633_Init(void)
{
  /* 0x68: setup register, internal clock mode 10,
     internal 2.5 V reference always on (MAX11633). */
  const uint8_t setup_command = 0x68U;
...
static HAL_StatusTypeDef MAX11633_Read16(uint16_t *samples)
{
  uint8_t conversion_command = 0xF8U; /* AIN0..AIN15 scan. */
...
static HAL_StatusTypeDef FSR_ScanFrame(uint16_t frame[FSR_ROWS][FSR_COLUMNS])
{
  ...
  {
    MUX1_SelectRow(row);
    HAL_Delay(1U); /* Allow the MUX and ADC input network to settle. */

    HAL_GPIO_WritePin(GPIOA, MUX_EN1_Pin, GPIO_PIN_RESET);
    status = MAX11633_Read16(frame[row]);
    HAL_GPIO_WritePin(GPIOA, MUX_EN1_Pin, GPIO_PIN_SET);
    if (status != HAL_OK)
    {
      return status;
...
static HAL_StatusTypeDef HOST_SendFrame(const uint16_t frame[FSR_ROWS][FSR_COLUMNS])
{
  ...
  /* IRQ high tells the Teensy that one complete frame is ready. */
  HAL_GPIO_WritePin(GPIOB, HOST_IRQ_Pin, GPIO_PIN_SET);
  status = HAL_SPI_TransmitReceive(&hspi3, host_tx_frame, host_rx_frame,
